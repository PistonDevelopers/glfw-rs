...
fn main() {
    ...
    do glfw::spawn {
        ...
        do window.spawn_pos_listener |msg| {
            ...
            // msg.window.set_title(fmt!("Window pos: [%d, %d]", msg.x, msg.y));
        }
        ...
        do window.spawn_key_listener |msg| {
            if msg.action == glfw::PRESS {
                match msg.key {
                    ...
                    k if k == glfw::KEY_UP     => { move_window(&msg.window, 0, 5);    }
                    k if k == glfw::KEY_DOWN   => { move_window(&msg.window, 0,-5);    }
                    k if k == glfw::KEY_LEFT   => { move_window(&msg.window,-5, 0);    }
                    k if k == glfw::KEY_RIGHT  => { move_window(&msg.window, 5, 0);    }
                    _ => {}
                }
            }
        }
